...
public class BiomeExtremeHillsEdge extends BiomeBase {
    ...
    public BiomeExtremeHillsEdge(int id) {
        super(id);
    }
    ...
    public int getMaxY(World world, Random random, int realX, int realZ, GenLayer layer) {
        int maxY = WATER_LEVEL + (int)Math.round(48*convertValue(BiomeBase.simplex[1].noise(realX, realZ, FREQ, AMP))) + 20;
    ...
    public void generateColumn(World world, Random random, byte[][] chunk, int realX, int realZ, int x, int z, GenLayer layer) {
        ...
        for(;y < maxY - random.nextInt(3) - 3 ; y++) {
